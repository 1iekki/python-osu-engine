    ...
    def render_hitobjects(self):
        if self.rendered < len(self.hitObjects):
            ...
            if hitTime <= musicTime:
                pos_x = self.window.center[0] - (self.playField[0] / 2)
                pos_y = self.window.center[1] - (self.playField[1] / 2)
                x = self.hitObjects[self.rendered].x + pos_x
                y = self.hitObjects[self.rendered].y + pos_y
...
class LevelSelection:
    ...
    def run(self):
        self.screen.fill(pygame.Color("Black"))
        for map in self.beatmaps:
            self.playMap.set_map(map)  
...
class Game:
    def __init__(self):
        ...
        self.playMap = PlayMap(self.screen, self.gameState,
                               self.clock, self.fpsCap,
                               self.settings['PLAYFIELD_DIMENSIONS'],
